...
function commitEdit(){
  ...
  if(sym){
    const t=inlineInp.value.trim(); if(t) sym.text=t;
...
function evalE(expr,vars){try{let e=expr.replace(/←/g,'=').replace(/\bMOD\b/gi,'%').replace(/\bDIV\b/gi,'/').replace(/\bAND\b/gi,'&&').replace(/\bOR\b/gi,'||').replace(/\bNOT\s+/gi,'!');for(const k in vars)e=e.replace(new RegExp('\\b'+k+'\\b','g'),JSON.stringify(vars[k]));return Function('"use strict";return('+e+')')();}catch{return expr;}}
